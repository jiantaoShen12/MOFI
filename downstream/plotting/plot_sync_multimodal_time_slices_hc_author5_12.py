...
import yaml
import umap.umap_ as umap               # 替换 PCA


def _install_serial_pynndescent_leaf_builder() -> None:
    """Keep the original UMAP fit usable in restricted Windows runtimes.

    pynndescent hard-codes a joblib thread pool while assembling RP-tree
    leaves.  Some notebook sandboxes deny the Windows pipe used by that pool.
    The serial implementation computes the same leaf arrays in the same tree
    order; it changes execution only, not the fitted data or UMAP parameters.
    """
    import pynndescent.rp_trees as rp_trees

    def _serial(rp_forest):
        max_leaf_size = np.max([rp_tree.leaf_size for rp_tree in rp_forest])
        return [
            rp_trees.get_leaves_from_tree(rp_tree, max_leaf_size)
            for rp_tree in rp_forest
        ]

    rp_trees.rptree_leaf_array_parallel = _serial
...
def render(**overrides: Any) -> Path:
    """Render HC author-5 time slices from Python without a CLI process."""

    args = parse_args([])
    for key, value in overrides.items():
        if not hasattr(args, key):
            raise TypeError(f"Unknown HC time-slice option: {key}")
...
def main(args: Optional[argparse.Namespace] = None) -> None:
    ...
    # ================== 替换 PCA 为 UMAP ==================
    _install_serial_pynndescent_leaf_builder()
    umap_main = umap.UMAP(
        n_components=2,
    ...
    for col, t in enumerate(display_times):
        item = slices[float(t)]
        # 使用 UMAP transform 代替 PCA transform
        main_coords = umap_main.transform(np.asarray(item["main_latent"], dtype=np.float32))
        sub_coords = umap_sub.transform(np.asarray(item["sub_latent"], dtype=np.float32))

        # 绘制 RNA 面板（上层）
        ax_rna = axes[0][col]
        _draw_panel(ax_rna, main_coords, np.asarray(item["main_labels"]).astype(str), f"RNA t={float(t):g}", is_real=bool(item["is_real"]))
        # 设置上层背景色
        if item["is_real"]:
            ax_rna.set_facecolor("#E8EAEC")   # 真实时间点
        else:
            ax_rna.set_facecolor("#F1F4F9")   # 插值点

        # 绘制 ATAC 面板（下层）
        ax_atac = axes[1][col]
        _draw_panel(ax_atac, sub_coords, np.asarray(item["sub_labels"]).astype(str), f"ATAC t={float(t):g}", is_real=bool(item["is_real"]))
        # 设置下层背景色
        if item["is_real"]:
            ax_atac.set_facecolor("#E9E4DB")   # 真实时间点
        else:
            ax_atac.set_facecolor("#F8F5ED")   # 插值点
    ...
    pdf_path = out_dir / "author5_time_slice_panels_umap.pdf"
    png_path = out_dir / "author5_time_slice_panels_umap.png"
