...
class HyperNetwork(nn.Module):
    def __init__(
        ...
        self._initialize_output(output_init)
    ...
    def _initialize_weights(self) -> None:
        ...
        if self.init_method not in valid_methods:
            raise ValueError(
                f"Weight init '{self.init_method}' not recognized. "
                f"Supported: {sorted(valid_methods)} + ['default']."
            )
        ...
        for module in self.modules():
            ...
            if self.init_method == 'xavier_uniform':
                ...
                init.kaiming_uniform_(
                    module.weight,
                    a=0.01 if self.activation == 'leaky_relu' else 0.0,
    ...
    def _initialize_output(self, output_init: Optional[str]) -> None:
        if output_init is None:
            return
        if output_init != 'zero':
            raise ValueError("output_init currently supports only 'zero'.")
