...
def compute_uot_plans(
    ...
    device: Optional[torch.device] = None  # Device for computation (e.g., cuda, mps, cpu). If None, auto-detect.
    ...
    Args:
        ...
        device: Device for computation (e.g., torch.device('cuda'), torch.device('mps'), torch.device('cpu')).
    ...
    uot_plans = []
    sampling_info_plans = []
    # Use provided device or fall back to auto-detection
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def get_batch_uot_fm(FM: ConditionalRegularizedUnbalancedFlowMatcher, X: list[np.ndarray], 
                    ...
                    device: Optional[torch.device] = None) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
