...
def preprocess(
    ...
    if dim_reduction.lower() == 'pca':
        pca_input_X = adata.X.copy()
        # Mean of PCA input space, used for approximate reconstruction:
        # X_recon ≈ X_pca @ reconstruction_matrix + input_mean
        pca_input_mean = np.asarray(adata.X.mean(axis=0)).ravel()
        sc.pp.pca(adata, n_comps=n_pcs, svd_solver='arpack')
        adata.obsm['X_latent'] = adata.obsm['X_pca']
        n_dim = adata.obsm['X_pca'].shape[1]
        if 'PCs' in adata.varm:
            projection_matrix = np.asarray(adata.varm['PCs'][:, :n_dim], dtype=np.float32).copy()
            reconstruction_matrix = np.asarray(np.linalg.pinv(projection_matrix), dtype=np.float32)
            adata.uns.setdefault('pca', {})
            # High-dimensional feature space -> PCA latent space.
            adata.uns['pca']['projection_matrix'] = projection_matrix
            # PCA latent space -> high-dimensional feature space (pseudo-inverse for best least-squares reconstruction).
            adata.uns['pca']['reconstruction_matrix'] = reconstruction_matrix
            adata.uns['pca']['input_mean'] = pca_input_mean
        # Create n_pc_adata
    ...
    if dim_reduction.lower() in ['pca', 'umap']:
        ...
        # Store the matrix before PCA when using PCA reduction
        if dim_reduction.lower() == 'pca' and pca_input_X is not None:
            original_gene_info['X_before_pca'] = pca_input_X
